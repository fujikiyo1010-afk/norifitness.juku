...
export function BodyRecordCardV2({ bodyCard }: { bodyCard: BodyCard }) {
  ...
  return (
    <div className="overflow-hidden rounded-[16px] border-2 border-[#4a875b] bg-[#fffdf8]">
      {/* 緑ヘッダー帯 */}
      <div
        className="flex items-start justify-between gap-2 px-[14px] py-[7px]"
      ...
      {/* 2大数字(体重=黒 / ウエスト=緑) — サイズ維持・上下だけしっかり詰める */}
      <div className="flex px-[15px] pb-[6px] pt-[8px]">
      ...
      {/* チップ4つ(表示だけ・非活性) — しっかり縦短く */}
      <div className="flex gap-[7px] px-[13px] pb-[8px]">
        {chips.map((c) => (
          <div
            key={c.label}
            className="flex flex-1 flex-col items-center gap-0 rounded-[10px] border border-[#e3d9c6] bg-[#fffef9] px-0.5 py-[3px]"
          >
      ...
      {/* ボタン2つ */}
      <div className="flex flex-col gap-1.5 px-[13px] pb-[10px]">
        {/* 上=記録シートをその場でせり上げ(案A・＋ボタンと同じ) — しっかり縦短く */}
        <button
          type="button"
          onClick={() => setRecordOpen(true)}
          className="flex items-center justify-center gap-1.5 rounded-[12px] py-[9px] text-[14px] font-extrabold text-white"
          style={{
        ...
        {/* 下=体組成ページ(グラフ) — しっかり縦短く */}
        <Link
          href="/record"
          className="flex items-center justify-center gap-1.5 rounded-[12px] border border-[#dbe8df] bg-white py-[6px] text-[12.5px] font-extrabold text-[#4a875b]"
        >
