...
import { BodyRecordCardV2 } from "@/app/BodyRecordCardV2";
      ...
      <div className="mx-auto w-full max-w-[460px] flex flex-1 flex-col border-x border-[#e7dcc9]">
        ...
        <section className={`bg-gradient-to-br from-[#e0f2f1] to-[#fffbe6] px-5 ${newHomeCard ? "py-2" : "py-3.5"}`}>
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            {streakDays > 0 && (
              <div className={`flex flex-shrink-0 items-center gap-1.5 rounded-full bg-[#fff3e0] ${newHomeCard ? "px-2.5 py-0.5" : "px-3 py-1.5"}`}>
                <FlameIcon size={newHomeCard ? 13 : 18} />
                <span className={`whitespace-nowrap font-extrabold text-[#c2693f] ${newHomeCard ? "text-[12.5px]" : "text-[16px]"}`}>
                  継続 {streakDays} 日目
                </span>
              </div>
            )}
        ...
        <div className="px-4 pt-0.5 pb-4 flex flex-col gap-2">
          ...
          {/* 今のからだ。newHomeCard(4人先行)=リッチ版(案C+縦詰め・上ボタンで記録シート) / それ以外=従来カード */}
          {newHomeCard ? (
            <BodyRecordCardV2 bodyCard={bodyCard} />
          ) : (
