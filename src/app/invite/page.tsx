  ...
  return (
    <main className="flex flex-1 flex-col items-center justify-center p-6">
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="rounded-md border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 p-4 text-sm space-y-1">
          <div className="text-zinc-500 dark:text-zinc-400">招待先</div>
  ...
  return (
    <main className="flex flex-1 flex-col items-center justify-center p-6">
      <div className="w-full max-w-md space-y-4 text-center">
        <h1 className="text-xl font-bold text-zinc-900 dark:text-zinc-50">⚠️ {title}</h1>
        <p className="text-sm text-zinc-600 dark:text-zinc-400">{message}</p>
        {showLoginLink && (
          <a
            href="/login"
            className="inline-block rounded-md bg-zinc-900 dark:bg-zinc-50 px-4 py-2 text-sm font-medium text-white dark:text-zinc-900"
          >
            ログイン画面へ
          </a>
        )}
