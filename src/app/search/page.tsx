    ...
    <>
      ...
      <main className="flex flex-1 flex-col p-6 sm:p-8">
        ...
        <div className="pt-2 border-t border-zinc-200 dark:border-zinc-800">
          <Link
            href="/courses"
            className="text-sm text-zinc-700 dark:text-zinc-300 underline hover:text-[#2b2620] dark:hover:text-zinc-50"
          >
            ← コース一覧へ戻る
          </Link>
        </div>
...
function ZeroResults({ query }: { query: string }) {
  return (
    <div className="rounded-lg border border-zinc-200 dark:border-zinc-800 bg-[#fffdf8] dark:bg-zinc-900 p-6 text-center space-y-3">
      ...
      <Link
        href="/courses"
        className="inline-block text-sm text-[#2b2620] dark:text-zinc-50 underline"
      >
        → コース一覧に戻る
      </Link>
    </div>
