...
test.beforeEach(async () => {
  ...
  // reset は shipments を消す(=入会直後)。本テストはホーム(/)を検証するため、
  // オンボ完了状態(shipments 行)に戻す。これが無いと / が /onboarding に飛ぶ。
...
test("17. 体組成 7 日途絶アラート: 受講生バナー + サブタイトル + SVG + 管理者除外", async ({
  ...
  try {
    ...
    // =====================================================================
    // 「変化を見る」リンクは /record (あすけん風グラフ embed) に統合済み。
    // /record は下部ナビ「記録」タブと同 href のため、テキストで絞る。
    const changeButton = studentPage
      .locator('a[href="/record"]')
      .filter({ hasText: "変化を見る" });
    ...
    // =====================================================================
    // 4. 管理者ホームダッシュ: 体組成途絶アラートは表示されない
    //    (2026-06-25 きよむ判断で管理側アラートから削除。受講生に 8 日途絶があっても
    //     管理ダッシュ・受講生一覧・ハブに「途絶」ラベルを出さない)
    //    ※カルテ未記入 / 目標シート空 など他アラートは引き続き出る (別機能)
    // =====================================================================
    await adminPage.goto("/admin");
    await expect(adminPage.locator("body")).not.toContainText("途絶");
  } finally {
